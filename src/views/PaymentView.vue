<template>
          ...
          <div class="col-md-6">
            ...
            <div v-else>
              <div v-for="item in cart" :key="item.id" class="cart-item">
                <img :src="item.imagem1" :alt="item.nome" class="cart-image" />
                <div>
                  <h5>{{ item.nome }}</h5>
                  <p class="text-muted">{{ item.seller }}</p>
                  <p class="fw-bold">{{ formatPrice(item.preco) }}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const total = computed(() => cartStore.cart.reduce((acc, item) => acc + item.preco, 0));
      ...
      return { cart, form, total, formatPrice, finalizarCompra };
    }
  });
  </script>
